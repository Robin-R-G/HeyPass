...
export async function GET(request: NextRequest) {
  return withAuth(request, async (req, userId, clientId) => {
    try {
      ...
      const completedCount = filtered.filter((p: any) => p.status === 'completed').length;
      const failedCount = filtered.filter((p: any) => p.status === 'failed').length;
      const refundedCount = filtered.filter((p: any) => p.status === 'refunded').length;

      const bankTotal = filtered
        .filter((p: any) => p.payment_methods?.method_type === 'bank_account')
        .reduce((sum: number, p: any) => sum + (parseFloat(p.amount) || 0), 0);
      const upiTotal = filtered
        .filter((p: any) => p.payment_methods?.method_type === 'upi')
        .reduce((sum: number, p: any) => sum + (parseFloat(p.amount) || 0), 0);
      ...
      const summaryRows = [
        ...
        `Bank Transfer Total,"₹${bankTotal.toLocaleString('en-IN')}"`,
        `UPI Total,"₹${upiTotal.toLocaleString('en-IN')}"`,
        '',
      ];

...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const sponsors = await sponsorAnalyticsService.getSponsors(auth.clientId, params.id);
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const sponsor = await sponsorAnalyticsService.createSponsor(auth.clientId, params.id, body);
